...
@Component({
  selector: "app-mun-upload-form",
  templateUrl: "./mun-upload-form.component.html",
  styleUrls: ["./mun-upload-form.component.scss"]
})
export class MunUploadFormComponent implements OnInit {
  festForm: FormGroup;
  submitted = false;
  ...
  ngOnInit() {
    ...
    this.festForm = this.formBuilder.group({
      ...
      committee: this.formBuilder.array([
  ...
  get committeePoints() {
    return this.festForm.get("committee") as FormArray;
  }
  ...
  deleteCommitteePoint(index) {
    this.committeePoints.removeAt(index);
  }

  addSponsorPoint() {
    this.sponsorPoints.push(
      this.formBuilder.group({
        evtSpnName: "",
  ...
  deleteSponsorEventPoint(index) {
    this.sponsorPoints.removeAt(index);
  }
